...
def book_details(id):
    ...
    user_books = []
    if len(rows) == 0:
        query = "SELECT name FROM books WHERE book_id = %s"
        cursor.execute(query, (id,))
        book_name = cursor.fetchone()[0]
    for row in rows:
        # desire format
        # fullname, book_id, book_name, category borrow_date, due_date, submitted_date
        # query to get  desired format by joining tables
        try:
            query = """
